...
def save_model(state_dict, filepath="microgpt_weights.pkl"):
    # Extract only raw numbers from Value objects
    raw_state = {}
...
# def run_inference(temperature=0.5, n_samples=20):
#     print("\n--- inference ---")
...
# 
def main():
    parser = argparse.ArgumentParser(description="MicroGPT CLI")
    parser.add_argument("-t", "--train", action="store_true", help="Train model, save, and infer")
    parser.add_argument("-i", "--infer", action="store_true", help="Load model and infer only")
    ...
    if args.train:
        print("\n🚀 Training model...")
        train_model()
        run_inference(temperature=args.temp)
        ...
        print("\n❌ Please specify -t (train) or -i (infer)")
        print("Example:")
        print("  python microgpt.py -t")
        print("  python microgpt.py -i")
